...
Use this script to download the latest Sila release binaries.
Downloaded binaries are saved to .\dist

To specify a specific release version:
  `$env:USE_SILA_VERSION="v1.0.0-beta.3"
 to resume using the latest release:
  Remove-Item env:USE_SILA_VERSION

To automatically restart crashed processes:
  `$env:SILA_AUTORESTART=`$TRUE ; .\sila.sh1 beacon-chain
 to stop autorestart run:
  Remove-Item env:SILA_AUTORESTART
"@;
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Host "ERROR: sila is only supported on 64-bit Operating Systems" -ForegroundColor Red;
    exit;
}
...
# Determine the latest release version, unless specified by $USE_SILA_VERSION.
if (Test-Path env:USE_SILA_VERSION) {
    Write-Host "Detected variable `$env:USE_SILA_VERSION=$env:USE_SILA_VERSION";
    $version = $env:USE_SILA_VERSION;
}
else {
    try {
        ...
        Write-Host "Using (latest) sila version: $version";
    }
    catch {
...
elseif ((Test-Path env:SILA_ALLOW_UNVERIFIED_BINARIES) -and $env:SILA_ALLOW_UNVERIFIED_BINARIES -eq $TRUE) {
    Write-Host "WARN Failed to verify sila binary" -ForegroundColor Yellow;
    Write-Host "Detected `$env:SILA_ALLOW_UNVERIFIED_BINARIES=`$TRUE";
    Write-Host "Proceeding...";
}
else {
    Write-Host "ERROR Failed to verify sila binary" -ForegroundColor Red;
    Write-Host @"
Please erase downloads in the
dist directory and run this script again. Alternatively, you can use a
a prior version by specifying environment variable `$env:USE_SILA_VERSION
with the specific version, as desired. Example: `$env:USE_SILA_VERSION=v1.0.0-alpha.5
If you must wish to continue running an unverified binary, use:
`$env:SILA_ALLOW_UNVERIFIED_BINARIES=`$TRUE
"@;
...
do {
    Write-Host "Starting: sila $($args -join ' ')";
    ...
    $restart = (Test-Path env:SILA_AUTORESTART) -and $env:SILA_AUTORESTART -eq $TRUE -and $process.ExitCode -ne 0;
} while ($restart)