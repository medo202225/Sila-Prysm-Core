# Benchmarks for Sila State Transition
This package contains the functionality needed for benchmarking Sila state transitions, this includes its block processing (with and without caching) and epoch processing functions. There is also a benchmark for HashTreeRoot on a large beacon state.
...
To generate new files to use for benchmarking, run the below command in the root of Sila.

```
bazel run //tools/benchmark-files-gen -- --output-dir $SILAPATH/testing/benchmark/benchmark_files/ --overwrite
```
