...
// NodeSet represents a set of SilaExecution nodes, none of which is a mining node.
type NodeSet struct {
...
// NewNodeSet creates and returns a set of SilaExecution nodes.
...
func (s *NodeSet) Start(ctx context.Context) error {
	...
	// We start up one SilaExecution node less than the beacon node count because the first
	// beacon node will connect to the already existing SilaExecution miner.
