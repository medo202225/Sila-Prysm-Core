...
func RunSilaExecutionDataResetTests(t *testing.T, config string) {
	...
	testFolders, testsFolderPath := utils.TestFolders(t, config, "altair", "epoch_processing/sila_execution_data_reset/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, "altair", "epoch_processing/sila_execution_data_reset/pyspec_tests")
	}
