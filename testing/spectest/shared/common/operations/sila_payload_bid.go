...
func runSilaPayloadBidTest(t *testing.T, config string, fork string, objName string, block blockWithSSZObject, sszToState SSZToState, operationFn BlockOperation) {
...
func RunSilaPayloadBidTest(t *testing.T, config string, fork string, block blockWithSSZObject, sszToState SSZToState) {
	runSilaPayloadBidTest(t, config, fork, "sila_payload_bid", block, sszToState, func(ctx context.Context, s state.BeaconState, b interfaces.ReadOnlySignedBeaconBlock) (state.BeaconState, error) {
		err := gloas.ProcessSilaPayloadBid(s, b.Block())
		return s, err
	})
}
