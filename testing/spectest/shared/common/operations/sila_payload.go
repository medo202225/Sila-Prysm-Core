...
func RunSilaPayloadTest(t *testing.T, config string, fork string, sszToBlockBody SSZToBlockBody, sszToState SSZToState) {
	require.NoError(t, utils.SetConfig(t, config))
	cfg := params.BeaconConfig()
	fv, err := version.FromString(fork)
	require.NoError(t, err)
	params.SetGenesisFork(t, cfg, fv)
	testFolders, testsFolderPath := utils.TestFolders(t, config, fork, "operations/sila_payload/pyspec_tests")
	if len(testFolders) == 0 {
		t.Fatalf("No test folders found for %s/%s/%s", config, fork, "operations/sila_payload/pyspec_tests")
	}
