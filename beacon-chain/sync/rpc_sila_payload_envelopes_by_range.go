...
// silaPayloadEnvelopesByRangeRPCHandler looks up the request sila payload envelopes from
// the database for the given slot range, serving one envelope per canonical block slot.
func (s *Service) silaPayloadEnvelopesByRangeRPCHandler(ctx context.Context, msg any, stream libp2pcore.Stream) error {
	ctx, span := trace.StartSpan(ctx, "sync.SilaPayloadEnvelopesByRangeHandler")
	defer span.End()
	recordResult := func(result silaPayloadEnvelopeRPCResult) {
		gloasSilaPayloadEnvelopesRPCRequestsTotal.WithLabelValues("by_range", string(result)).Inc()
		if result == silaPayloadEnvelopeRPCResultServed {
	...
	log := log.WithField("handler", p2p.SilaPayloadEnvelopesByRangeName[1:])

	r, ok := msg.(*pb.SilaPayloadEnvelopesByRangeRequest)
	if !ok {
		recordResult(silaPayloadEnvelopeRPCResultInvalid)
		return errors.New("message is not type *pb.SilaPayloadEnvelopesByRangeRequest")
	}
	if err := s.rateLimiter.validateRequest(stream, 1); err != nil {
		recordResult(silaPayloadEnvelopeRPCResultRateLimited)
		return err
	}
	...
	log.WithFields(logrus.Fields{
		...
		"count":     r.Count,
		"peer":      remotePeer,
	}).Debug("Serving sila payload envelopes by range request")

	rp, err := validateEnvelopesByRange(r, s.cfg.clock.CurrentSlot())
	if err != nil {
		recordResult(silaPayloadEnvelopeRPCResultInvalid)
		s.writeErrorResponseToStream(responseCodeInvalidRequest, err.Error(), stream)
		s.downscorePeer(remotePeer, "silaPayloadEnvelopesByRangeRPCHandlerValidationError")
		tracing.AnnotateError(span, err)
		return err
	}
	available := s.validateRangeAvailability(rp)
	if !available {
		recordResult(silaPayloadEnvelopeRPCResultResourceUnavailable)
		currentSlot := s.cfg.clock.CurrentSlot()
		unavailableErr := errors.Wrapf(
			p2ptypes.ErrResourceUnavailable,
			"sila payload envelope range unavailable start=%d end=%d current=%d",
			rp.start,
			rp.end,
			currentSlot,
		)
		log.WithFields(logrus.Fields{
			...
			"size":      rp.size,
			"current":   currentSlot,
		}).WithError(unavailableErr).Debug("Sila payload envelope range unavailable")
	...
	if err := s.streamCanonicalEnvelopes(ctx, rp, stream); err != nil {
		recordResult(silaPayloadEnvelopeRPCResultError)
		tracing.AnnotateError(span, err)
		return err
	}

	recordResult(silaPayloadEnvelopeRPCResultServed)
...
func (s *Service) streamCanonicalEnvelopes(ctx context.Context, rp rangeParams, stream libp2pcore.Stream) error {
	...
	type collectedEnvelope struct {
		env       *pb.SignedBlindedSilaPayloadEnvelope
		blockHash [32]byte
	}
	...
	bid, err := successorBlock.Block().Body().SignedSilaPayloadBid()
	if err != nil {
		s.writeErrorResponseToStream(responseCodeServerError, p2ptypes.ErrGeneric.Error(), stream)
	...
	for {
		...
		blindedEnv, err := s.cfg.beaconDB.SilaPayloadEnvelopeByBlockHash(ctx, parentBlockHash)
		if err != nil {
			log.WithError(err).WithField("blockHash", bytesutil.Trunc(parentBlockHash[:])).Debug("Could not load sila payload envelope")
			break
		}
	...
	payloadByHash, err := s.cfg.executionReconstructor.ReconstructFullGloasSilaPayloadsByHash(ctx, batchHashes)
	if err != nil {
		s.writeErrorResponseToStream(responseCodeServerError, p2ptypes.ErrGeneric.Error(), stream)
		return errors.Wrap(err, "could not batch reconstruct full sila payload envelopes")
	}
	...
	for _, c := range collected {
		...
		if chunkErr := WriteSilaPayloadEnvelopeChunk(stream, s.cfg.p2p.Encoding(), fullEnv); chunkErr != nil {
			log.WithError(chunkErr).Debug("Could not send sila payload envelope chunk")
			s.writeErrorResponseToStream(responseCodeServerError, p2ptypes.ErrGeneric.Error(), stream)
...
// validateEnvelopesByRange validates the SilaPayloadEnvelopesByRange request and returns
// normalized rangeParams. Mirrors validateBlobsByRange in structure.
func validateEnvelopesByRange(r *pb.SilaPayloadEnvelopesByRangeRequest, current primitives.Slot) (rangeParams, error) {
	if r.Count == 0 {
