...
// silaPayloadEnvelopesByRootRPCHandler handles the
// /sila/beacon_chain/req/sila_payload_envelopes_by_root/1/ RPC request.
...
func (s *Service) silaPayloadEnvelopesByRootRPCHandler(ctx context.Context, msg any, stream libp2pcore.Stream) error {
	ctx, span := trace.StartSpan(ctx, "sync.silaPayloadEnvelopesByRootRPCHandler")
	defer span.End()
	recordResult := func(result silaPayloadEnvelopeRPCResult) {
		gloasSilaPayloadEnvelopesRPCRequestsTotal.WithLabelValues("by_root", string(result)).Inc()
	...
	log := log.WithField("handler", p2p.SilaPayloadEnvelopesByRootName[1:]) // slice the leading slash off the name var

	ref, ok := msg.(*types.SilaPayloadEnvelopesByRootReq)
	if !ok {
		recordResult(silaPayloadEnvelopeRPCResultInvalid)
		return errors.New("message is not type SilaPayloadEnvelopesByRootReq")
	}
	...
	if err := validateSilaPayloadEnvelopeByRootRequest(len(requestedRoots)); err != nil {
		recordResult(silaPayloadEnvelopeRPCResultInvalid)
		s.downscorePeer(remotePeer, "silaPayloadEnvelopesByRootRPCHandlerValidationError")
	...
	type requestedEnvelope struct {
		root [32]byte
		env  *silapb.SignedBlindedSilaPayloadEnvelope
	}
	...
	for start := 0; start < len(requestedRoots); start += batchSize {
		...
		for _, root := range rootsBatch {
			...
			blindedEnvelope, err := s.cfg.beaconDB.SilaPayloadEnvelope(ctx, root)
			if err != nil {
				if errors.Is(err, db.ErrNotFound) {
					log.WithField("root", fmt.Sprintf("%#x", root)).Trace("Peer requested sila payload envelope by root not found")
					continue
				}
				log.WithError(err).Debug("Could not fetch blinded sila payload envelope")
				recordResult(silaPayloadEnvelopeRPCResultError)
				s.writeErrorResponseToStream(responseCodeServerError, types.ErrGeneric.Error(), stream)
				return err
			}
		...
		payloadByHash, batchErr := s.cfg.executionReconstructor.ReconstructFullGloasSilaPayloadsByHash(ctx, batchHashes)
		if batchErr != nil {
			recordResult(silaPayloadEnvelopeRPCResultError)
			log.WithError(batchErr).Debug("Could not batch reconstruct full sila payload envelopes")
			s.writeErrorResponseToStream(responseCodeServerError, types.ErrGeneric.Error(), stream)
			return batchErr
		}
		...
		for _, req := range requestedEnvs {
			...
			if chunkErr := WriteSilaPayloadEnvelopeChunk(stream, s.cfg.p2p.Encoding(), envelope); chunkErr != nil {
...
// validateSilaPayloadEnvelopeByRootRequest checks if the request for sila payload envelopes is valid.
func validateSilaPayloadEnvelopeByRootRequest(count int) error {
	if count == 0 {
