...
func (s *Service) validateSilaPayloadEnvelope(ctx context.Context, pid peer.ID, msg *pubsub.Message) (pubsub.ValidationResult, error) {
	...
	ctx, span := trace.StartSpan(ctx, "sync.validateSilaPayloadEnvelope")
	defer span.End()
	...
	signedEnvelope, ok := m.(*silapb.SignedSilaPayloadEnvelope)
	if !ok {
		return pubsub.ValidationReject, errWrongMessage
	}
	e, err := blocks.WrappedROSignedSilaPayloadEnvelope(signedEnvelope)
	if err != nil {
		log.WithError(err).Error("failed to create read only signed payload execution envelope")
		return pubsub.ValidationIgnore, err
	}
	v := s.newSilaPayloadEnvelopeVerifier(e, verification.GossipSilaPayloadEnvelopeRequirements)
	...
	// [IGNORE] The node has not seen another valid SignedSilaPayloadEnvelope for this block root from this builder.
	...
	// Let bid alias block.body.signed_sila_payload_bid.message
	// (notice that this can be obtained from the state.latest_sila_payload_bid).
	signedBid, err := block.Block().Body().SignedSilaPayloadBid()
	if err != nil {
		return pubsub.ValidationIgnore, err
	}
	...
	// [REJECT] signed_sila_payload_envelope.signature is valid with respect to the builder's public key.
	...
	if err == nil {
		syncSilaPayloadEnvelopeArrivalDelaySeconds.Observe(receivedTime.Sub(startTime).Seconds())
	} else {
		log.WithError(err).WithField("slot", env.Slot()).Debug("Could not compute sila payload envelope slot start time")
	}

	// sila_payload_gossip fires once the envelope passes gossip validation.
...
func (s *Service) queuePendingPayloadEnvelope(
	ctx context.Context,
	v verification.SilaPayloadEnvelopeVerifier,
	env interfaces.ROSilaPayloadEnvelope,
	signedEnvelope *silapb.SignedSilaPayloadEnvelope,
...
func (s *Service) silaPayloadEnvelopeSubscriber(ctx context.Context, msg proto.Message) error {
	e, ok := msg.(*silapb.SignedSilaPayloadEnvelope)
	if !ok {
		return errWrongMessage
	}
	env, err := blocks.WrappedROSignedSilaPayloadEnvelope(e)
	if err != nil {
		return errors.Wrap(err, "could not wrap signed sila payload envelope")
	}
	if err := s.cfg.chain.ReceiveSilaPayloadEnvelope(ctx, env); err != nil {
		if blockchain.IsInvalidBlock(err) {
			envelope, envErr := env.Envelope()
			if envErr == nil {
				s.setBadPayload(ctx, envelope.BeaconBlockRoot())
			} else {
				log.WithError(envErr).Error("failed to get envelope from signed sila payload envelope")
			}
		}
		return err
	}
