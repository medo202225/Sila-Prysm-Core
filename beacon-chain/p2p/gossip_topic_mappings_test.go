...
func TestGossipTopicMappings_CorrectType(t *testing.T) {
	...
	assert.Equal(t, true, ok)
	pMessage = GossipTopicMappings(SilaPayloadBidTopicFormat, gloasForkEpoch)
	_, ok = pMessage.(*silapb.SignedSilaPayloadBid)
	assert.Equal(t, true, ok)
	assert.Equal(t, SilaPayloadBidTopicFormat, GossipTypeMapping[reflect.TypeFor[*silapb.SignedSilaPayloadBid]()])
