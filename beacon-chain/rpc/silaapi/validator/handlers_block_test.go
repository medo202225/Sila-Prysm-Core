...
func TestProduceBlockV3(t *testing.T) {
	...
	t.Run("Phase 0", func(t *testing.T) {
		...
		v1alpha1Server.EXPECT().GetBeaconBlock(gomock.Any(), &sila.BlockRequest{
			Slot:         1,
			RandaoReveal: bRandao,
			Graffiti:     bGraffiti,
			SkipMevBoost: false,
		}).Return(
			func() (*sila.GenericBeaconBlock, error) {
		...
		want := fmt.Sprintf(`{"version":"phase0","sila_payload_blinded":false,"sila_payload_value":"","consensus_block_value":"","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Altair", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"altair","sila_payload_blinded":false,"sila_payload_value":"","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Bellatrix", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"bellatrix","sila_payload_blinded":false,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("BlindedBellatrix", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"bellatrix","sila_payload_blinded":true,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "true", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
		require.Equal(t, "bellatrix", writer.Header().Get(api.VersionHeader))
		require.Equal(t, "10000000000", writer.Header().Get(api.ConsensusBlockValueHeader))
	})
	t.Run("Capella", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"capella","sila_payload_blinded":false,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Blinded Capella", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"capella","sila_payload_blinded":true,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "true", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Deneb", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"deneb","sila_payload_blinded":false,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Blinded Deneb", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"deneb","sila_payload_blinded":true,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "true", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Electra", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"electra","sila_payload_blinded":false,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Blinded Electra", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"electra","sila_payload_blinded":true,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "true", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Fulu", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"fulu","sila_payload_blinded":false,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("Blinded Fulu", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"fulu","sila_payload_blinded":true,"sila_payload_value":"2000","consensus_block_value":"10000000000","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "true", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
	...
	t.Run("0 block value is returned on error", func(t *testing.T) {
		...
		server.ProduceBlockV3(writer, request)
		assert.Equal(t, http.StatusOK, writer.Code)
		want := fmt.Sprintf(`{"version":"fulu","sila_payload_blinded":false,"sila_payload_value":"2000","consensus_block_value":"0","data":%s}`, string(jsonBytes))
		body := strings.ReplaceAll(writer.Body.String(), "\n", "")
		require.Equal(t, want, body)
		require.Equal(t, "false", writer.Header().Get(api.SilaPayloadBlindedHeader))
		require.Equal(t, "2000", writer.Header().Get(api.SilaPayloadValueHeader))
...
func TestProduceBlockV3SSZ(t *testing.T) {
	...
	t.Run("Phase 0", func(t *testing.T) {
		...
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericSignedBeaconBlock_Phase0)
	...
	t.Run("Altair", func(t *testing.T) {
		...
		g, err := block.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericSignedBeaconBlock_Altair)
		require.Equal(t, true, ok)
	...
	t.Run("Bellatrix", func(t *testing.T) {
		...
		g, err := block.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericSignedBeaconBlock_Bellatrix)
		require.Equal(t, true, ok)
	...
	t.Run("BlindedBellatrix", func(t *testing.T) {
		...
		g, err := block.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericSignedBeaconBlock_BlindedBellatrix)
		require.Equal(t, true, ok)
	...
	t.Run("Capella", func(t *testing.T) {
		...
		g, err := block.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericSignedBeaconBlock_Capella)
		require.Equal(t, true, ok)
	...
	t.Run("Blinded Capella", func(t *testing.T) {
		...
		g, err := block.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericSignedBeaconBlock_BlindedCapella)
		require.Equal(t, true, ok)
	...
	t.Run("Deneb", func(t *testing.T) {
		...
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericBeaconBlock_Deneb)
		require.Equal(t, true, ok)
	...
	t.Run("Blinded Deneb", func(t *testing.T) {
		...
		g, err := block.Message.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericBeaconBlock_BlindedDeneb)
		require.Equal(t, true, ok)
	...
	t.Run("Electra", func(t *testing.T) {
		...
		g, err := block.ToUnsigned().ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericBeaconBlock_Electra)
		require.Equal(t, true, ok)
	...
	t.Run("Blinded Electra", func(t *testing.T) {
		...
		g, err := block.Message.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericBeaconBlock_BlindedElectra)
		require.Equal(t, true, ok)
	...
	t.Run("Fulu", func(t *testing.T) {
		...
		g, err := block.ToUnsigned().ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericBeaconBlock_Fulu)
		require.Equal(t, true, ok)
	...
	t.Run("Blinded Fulu", func(t *testing.T) {
		...
		g, err := block.Message.ToGeneric()
		require.NoError(t, err)
		bl, ok := g.Block.(*sila.GenericBeaconBlock_BlindedFulu)
		require.Equal(t, true, ok)
