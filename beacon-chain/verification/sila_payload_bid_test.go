...
func TestBidVerifier_VerifyCurrentOrNextSlot(t *testing.T) {
	...
	currentBid := testSignedSilaPayloadBid(t, currentSlot)
	currentWrapped, err := blocks.WrappedROSignedSilaPayloadBid(currentBid)
	...
	nextBid := testSignedSilaPayloadBid(t, currentSlot+1)
	nextWrapped, err := blocks.WrappedROSignedSilaPayloadBid(nextBid)
	require.NoError(t, err)
	verifier = &BidVerifier{
		sharedResources: &sharedResources{clock: clock},
		results:         newResults(RequireBidCurrentOrNextSlot),
	...
	futureBid := testSignedSilaPayloadBid(t, currentSlot+2)
	futureWrapped, err := blocks.WrappedROSignedSilaPayloadBid(futureBid)
	require.NoError(t, err)
	verifier = &BidVerifier{
		sharedResources: &sharedResources{clock: clock},
		results:         newResults(RequireBidCurrentOrNextSlot),
...
func TestBidVerifier_VerifyGasLimitTargetCompatible(t *testing.T) {
	...
	// bid.gas_limit is 1 (from testSignedSilaPayloadBid). With parent=1 the
...
func TestBidVerifier_VerifyBidSlotHigherThanParent(t *testing.T) {
	signed := testSignedSilaPayloadBid(t, 10)
	wrapped, err := blocks.WrappedROSignedSilaPayloadBid(signed)
	require.NoError(t, err)
...
func TestBidVerifier_VerifySignature(t *testing.T) {
	...
	signed := testSignedSilaPayloadBid(t, 4)
...
func testSignedSilaPayloadBid(t *testing.T, slot primitives.Slot) *silapb.SignedSilaPayloadBid {
	t.Helper()
...
func signBidForState(t *testing.T, sk bls.SecretKey, bid *silapb.SilaPayloadBid, st state.ReadOnlyBeaconState) [96]byte {
	t.Helper()
