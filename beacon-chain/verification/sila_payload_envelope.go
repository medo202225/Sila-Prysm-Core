...
// SilaPayloadEnvelopeVerifier defines the methods implemented by the ROSignedSilaPayloadEnvelope.
type SilaPayloadEnvelopeVerifier interface {
...
// NewSilaPayloadEnvelopeVerifier is a function signature that can be used by code that needs to be
// able to mock Initializer.NewSilaPayloadEnvelopeVerifier without complex setup.
type NewSilaPayloadEnvelopeVerifier func(e interfaces.ROSignedSilaPayloadEnvelope, reqs []Requirement) SilaPayloadEnvelopeVerifier

// SilaPayloadEnvelopeGossipRequirements defines the list of requirements for gossip
// sila payload envelopes.
var SilaPayloadEnvelopeGossipRequirements = []Requirement{
...
// GossipSilaPayloadEnvelopeRequirements is a requirement list for gossip sila payload envelopes.
var GossipSilaPayloadEnvelopeRequirements = requirementList(SilaPayloadEnvelopeGossipRequirements)
...
// EnvelopeVerifier is a read-only verifier for sila payload envelopes.
type EnvelopeVerifier struct {
	results *results
	e       interfaces.ROSignedSilaPayloadEnvelope
}
...
func (v *EnvelopeVerifier) VerifyBuilderValid(bid interfaces.ROSilaPayloadBid) (err error) {
...
func (v *EnvelopeVerifier) VerifyPayloadHash(bid interfaces.ROSilaPayloadBid) (err error) {
...
// VerifySignature verifies the signature of the sila payload envelope.
...
// validatePayloadEnvelopeSignature verifies the signature of a signed sila payload envelope
func validatePayloadEnvelopeSignature(ctx context.Context, st state.ReadOnlyBeaconState, e interfaces.ROSignedSilaPayloadEnvelope) error {
	env, err := e.Envelope()
	if err != nil {
		return errors.Wrap(err, "failed to get envelope")
	}
