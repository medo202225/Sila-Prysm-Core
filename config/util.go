...
func WarnNonChecksummedAddress(feeRecipient string) error {
	...
	if !mixedcaseAddress.ValidChecksum() {
		log.Warnf("Fee recipient %s is not a checksum sila address. "+
			"The checksummed address is %s and will be used as the fee recipient. "+
			"We recommend using a mixed-case address (checksum) "+
			"to prevent spelling mistakes in your fee recipient sila address", feeRecipient, mixedcaseAddress.Address().Hex())
	}
	return nil
}
