...
- `GET /sila/v1/validator/execution_payload_envelope/{slot}` →
  `GET /sila/v1/validator/execution_payload_envelopes/{slot}/{beacon_block_root}`;
...
- `POST /sila/v1/beacon/execution_payload_envelopes` body shape is now selected by
...
- Pluralized gloas execution payload endpoint paths to match the REST naming
  convention (beacon-APIs #613): `POST /sila/v1/beacon/execution_payload_bid` →
  `/sila/v1/beacon/execution_payload_bids`, and the execution payload envelope
