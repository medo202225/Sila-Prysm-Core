...
func TestValidHostname(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			joined:  "http://mydomain.org:3500/sila/v1/node/version",
		},
		{
			name:    "https scheme, hostname with port",
			hostArg: "https://mydomain.org:3500",
			path:    GetNodeVersionPath,
			joined:  "https://mydomain.org:3500/sila/v1/node/version",
		},
		{
			name:    "http scheme, hostname without port",
			hostArg: "http://mydomain.org",
			path:    GetNodeVersionPath,
			joined:  "http://mydomain.org/sila/v1/node/version",
		},
		{
		...
		},
		{
			...
			path:    GetNodeVersionPath,
			joined:  "http://username:pass@mydomain.org/sila/v1/node/version",
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
