...
(for /f %%i in ('curl -f -s https://prysmaticlabs.com/releases/latest') do set prysm_version=%%i) || (echo [31mERROR: Starting Sila-Prysm requires an internet connection. If you are being blocked by your antivirus, you can download the beacon chain and validator executables from our releases page on Github here https://github.com/sila-chain/Sila-Prysm-Core/releases/ [0m && exit /b 1)
...
if "%~1"=="slasher" (
    echo [31mThe slasher binary is no longer available. Please use the --slasher flag with your beacon node. See: https://github.com/sila-chain/Sila-Prysm-Core[0m
    exit /b 1
)
