...
// getSilaPayloadEnvelope returns the envelope to sign for self-build. Stateless mode has the
...
func (c *beaconApiValidatorClient) getSilaPayloadEnvelope(
	ctx context.Context,
	slot primitives.Slot,
	beaconBlockRoot [32]byte,
) (*silapb.SilaPayloadEnvelope, *silapb.WireBlindedSilaPayloadEnvelope, error) {
	if envelope, _, _ := c.envelopeCache.peek(slot); envelope != nil {
		return envelope, nil, nil
	}

	endpoint := fmt.Sprintf("/sila/v1/validator/sila_payload_envelopes/%d/%s", slot, hexutil.Encode(beaconBlockRoot[:]))
	body, header, err := c.handler.GetSSZ(ctx, endpoint)
	if err != nil {
		return nil, nil, errors.Wrap(err, "could not get blinded sila payload envelope")
	}
	...
	var resp structs.GetValidatorBlindedSilaPayloadEnvelopeResponse
	if err := json.Unmarshal(body, &resp); err != nil {
		return nil, nil, errors.Wrap(err, "could not decode blinded envelope JSON")
	}
	if resp.Data == nil {
		return nil, nil, errors.New("blinded sila payload envelope data is nil")
	}
...
// publishSilaPayloadEnvelope publishes the full envelope plus cached blobs/proofs as
// SignedSilaPayloadEnvelopeContents (stateless flow). Stateful self-build uses
// publishBlindedSilaPayloadEnvelope instead.
func (c *beaconApiValidatorClient) publishSilaPayloadEnvelope(
	ctx context.Context,
	envelope *silapb.SignedSilaPayloadEnvelope,
) (*empty.Empty, error) {
	const endpoint = "/sila/v1/beacon/sila_payload_envelopes"
	...
	contents := &silapb.SignedSilaPayloadEnvelopeContents{
		SignedSilaPayloadEnvelope: envelope,
		KzgProofs:                      kzgProofs,
		Blobs:                          blobs,
	}
	...
	jsonFn := func() ([]byte, error) {
		j, jerr := structs.SignedSilaPayloadEnvelopeContentsFromConsensus(envelope, kzgProofs, blobs)
		if jerr != nil {
			return nil, jerr
		}
	...
	if err := c.postEnvelope(ctx, endpoint, envelopeHeaders(false), ssz, jsonFn); err != nil {
		return nil, errors.Wrap(err, "could not publish sila payload envelope contents")
	}
	return &empty.Empty{}, nil
}

// publishBlindedSilaPayloadEnvelope publishes the signed blinded envelope (stateful flow); the
// BN reconstructs the full envelope from its cache. Signature is valid by HTR equivalence.
func (c *beaconApiValidatorClient) publishBlindedSilaPayloadEnvelope(
	ctx context.Context,
	signed *silapb.SignedWireBlindedSilaPayloadEnvelope,
) (*empty.Empty, error) {
	const endpoint = "/sila/v1/beacon/sila_payload_envelopes"
	...
	jsonFn := func() ([]byte, error) {
		msg, jerr := structs.BlindedSilaPayloadEnvelopeFromConsensus(signed.Message)
		if jerr != nil {
			return nil, jerr
		}
		j := &structs.SignedBlindedSilaPayloadEnvelope{
			Message:   msg,
	...
	if err := c.postEnvelope(ctx, endpoint, envelopeHeaders(true), ssz, jsonFn); err != nil {
		return nil, errors.Wrap(err, "could not publish blinded sila payload envelope")
	}
	return &empty.Empty{}, nil
}
...
func envelopeHeaders(blinded bool) map[string]string {
	return map[string]string{
		...
		api.SilaPayloadBlindedHeader: strconv.FormatBool(blinded),
	}
}
